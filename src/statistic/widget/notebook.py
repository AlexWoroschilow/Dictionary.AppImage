...
class StatisticPage(wx.Panel):
    ...
    def history(self, history):
        ...
        self.axes.set_xticklabels(labels, rotation=23, fontdict={'size': 5})
        self.axes.grid(b=True, which='major', color='#c0c0c0', linestyle='-')
 
        if len(labels) > 1 and len(values) > 1:
            self.axes.plot(positions, values, linewidth=3.0, color='green')

        self._label.SetLabelText("%s days history statistic" % len(labels))
